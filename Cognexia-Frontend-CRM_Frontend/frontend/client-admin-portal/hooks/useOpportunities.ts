...
export const useUpdateOpportunityStatus = () => {
  ...
  return useMutation({
    mutationFn: ({ id, status }: { id: string; status: 'open' | 'won' | 'lost' }) =>
      updateOpportunity(id, { status } as UpdateOpportunityDto),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: opportunityKeys.lists() });
      queryClient.invalidateQueries({ queryKey: opportunityKeys.detail(variables.id) });
      queryClient.invalidateQueries({ queryKey: opportunityKeys.stats() });
      queryClient.invalidateQueries({ queryKey: opportunityKeys.pipeline() });
      toast.success(`Opportunity marked as ${variables.status}`);
    },
    onError: (error: any) => {
