...
export function ContactForm({ initialData, onSubmit, isLoading, submitLabel = 'Create Contact' }: ContactFormProps) {
  ...
  const { data: customersData, isLoading: isLoadingCustomers } = useCustomers({ limit: 500 } as any);
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <SelectContent>
                        <div className="flex items-center gap-2 border-b px-3 pb-2 mb-1">
                        ...
                        {customers
                          ...
                          .map((customer) => (
                            <SelectItem key={customer.id} value={customer.id}>
                              {customer.companyName || `${customer.primaryContact?.firstName || ''} ${customer.primaryContact?.lastName || ''}`.trim() || customer.primaryContact?.email} ({customer.customerCode})
                            </SelectItem>
                          ))}
                        {customers.filter((c) => {
                          const q = customerSearch.toLowerCase();
                          return !q || (c.companyName || '').toLowerCase().includes(q) || (c.primaryContact?.email || '').toLowerCase().includes(q) || (c.customerCode || '').toLowerCase().includes(q);
                        }).length === 0 && (
