...
export default function ClickToCall({ defaultNumber = '', onStarted, onEnded, className }: ClickToCallProps) {
  const [toNumber, setToNumber] = useState(defaultNumber);
  // Add state for fromNumber if you want the user to type it in, OR just hardcode your app's main number
  ...
  const handleStart = () => {
    // Make sure we have both numbers before initiating
    if (!toNumber || !fromNumber) return;
    
    initiate.mutate(
      { 
        toNumber: toNumber, 
        fromNumber: fromNumber // <--- Add fromNumber here!
      },
      {
        onSuccess: (call) => {
          setActiveCall(call);
      ...
      <CardContent className="pt-6 space-y-4">
        {!activeCall ? (
          <div className="grid gap-3 md:grid-cols-2">
            <div>
              <Label htmlFor="fromNumber">From number (Your Number)</Label>
              <Input
                id="fromNumber"
            ...
            <div>
              <Label htmlFor="toNumber">To number (Destination)</Label>
              <Input
                id="toNumber"
                placeholder="e.g. +15551234567"
                value={toNumber}
            ...
            <div className="md:col-span-2">
              <Button 
                onClick={handleStart} 
                disabled={!toNumber || !fromNumber || calling} 
                className="w-full"
              >
                <Phone className="h-4 w-4 mr-2" />
                {calling ? 'Calling...' : 'Call'}
