# ========================================
# CognexiaAI CRM - Complete API Endpoint Tester
# ========================================
...
$testProductId = "00000000-0000-0000-0000-000000000002"
...
function Test-Endpoint {
    ...
    try {
        ...
        $responseJson = if ($null -ne $response) { 
            $response | ConvertTo-Json -Depth 2 -Compress 
        } else { 
            '' 
        }
        $responsePreview = if ([string]::IsNullOrEmpty($responseJson)) { 
            '' 
        } else { 
            $responseJson.Substring(0, [Math]::Min(100, $responseJson.Length)) 
        }
...
$registerBody = @{
    ...
    firstName = "Test"
    lastName = "User"
    organizationName = "Test Organization $((New-Guid).ToString().Substring(0, 8))"
}
...
$testResults += Test-Endpoint -Method "POST" -Url "$baseUrl/quantum/personality-profile" -Name "Generate Personality Profile" -Body @{customerId=$testCustomerId}
$testResults += Test-Endpoint -Method "GET" -Url "$baseUrl/quantum/entanglement/$testCustomerId" -Name "Analyze Customer Entanglement"
$testResults += Test-Endpoint -Method "POST" -Url "$baseUrl/quantum/consciousness-simulation" -Name "Simulate Consciousness" -Body @{customerId=$testCustomerId}
$testResults += Test-Endpoint -Method "GET" -Url "$baseUrl/quantum/behavioral-predictions/$testCustomerId" -Name "Predict Quantum Behavior"
$testResults += Test-Endpoint -Method "GET" -Url "$baseUrl/quantum/emotional-resonance/$testCustomerId" -Name "Analyze Emotional Resonance"
...
$testResults += Test-Endpoint -Method "POST" -Url "$baseUrl/holographic/projections" -Name "Create Holographic Projection" -Body @{customerId=$testCustomerId; type="PRODUCT_DEMO"}
$testResults += Test-Endpoint -Method "POST" -Url "$baseUrl/holographic/spatial-computing/start" -Name "Start Spatial Computing Session" -Body @{customerId=$testCustomerId}
...
$testResults += Test-Endpoint -Method "POST" -Url "$baseUrl/arvr/meetings" -Name "Schedule Virtual Meeting" -Body @{customerId=$testCustomerId; scheduledAt="2026-01-20T10:00:00Z"}
$testResults += Test-Endpoint -Method "POST" -Url "$baseUrl/arvr/product-demos" -Name "Create 3D Product Demo" -Body @{productId=$testProductId}
$testResults += Test-Endpoint -Method "POST" -Url "$baseUrl/arvr/configurator/initialize" -Name "Initialize 3D Configurator" -Body @{productId=$testProductId}
...
$testResults += Test-Endpoint -Method "POST" -Url "$baseUrl/contracts" -Name "Create Contract" -Body @{name="Service Agreement"; customerId=$testCustomerId; value=100000}
...
$testResults += Test-Endpoint -Method "POST" -Url "$baseUrl/llm/chat" -Name "Start Chat Conversation" -Body @{customerId=$testCustomerId; context="sales"}
...
$testResults += Test-Endpoint -Method "POST" -Url "$baseUrl/llm/sentiment" -Name "Analyze Sentiment" -Body @{customerId=$testCustomerId}
...
$csvPath = "C:\Users\nshrm\Desktop\CognexiaAI-ERP\api-test-results.csv"
