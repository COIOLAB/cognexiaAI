 ...
 * This guard extracts the organization_id from the JWT token and validates
 * that users can only access resources belonging to their organization.
 * 
 * Usage:
 * @UseGuards(JwtAuthGuard, TenantGuard)
 * 
 * The guard expects the organization_id to be present in the JWT payload.
 */
@Injectable()
export class TenantGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!organizationId) {
      this.logger.error(
        `User ${user.id} does not have an organization_id in token`,
      );
    ...
    // Attach organization_id to request for downstream use
