...
export class UserTierService {
   ...
   */
  async getUserTierAllocation(organization_id: string): Promise<UserTierAllocationDto> {
   ...
   */
  async canAddUser(organization_id: string): Promise<{ canAdd: boolean; reason?: string }> {
    const allocation = await this.getUserTierAllocation(organization_id);
  ...
  async validateUserAddition(organization_id: string): Promise<void> {
    const check = await this.canAddUser(organization_id);
   ...
   */
  async initializeDefaultTier(organization_id: string): Promise<void> {
    const organization = await this.organizationRepository.findOne({
      where: { id: organization_id },
    });
