...
export class AuditLogService {
  ...
  async findAll(filters: AuditLogFilterDto) {
    ...
    // Apply filters
    if (organizationId) {
      queryBuilder.andWhere('audit_log.organization_id = :organizationId', { organizationId });
    }
   ...
   */
  async findByOrganization(organization_id: string, filters: AuditLogFilterDto) {
    return this.findAll({ ...filters, organizationId: organization_id });
  }
  ...
  async exportToCsv(filters: AuditLogFilterDto): Promise<string> {
    ...
    data.forEach((log) => {
      csv += `"${log.id}","${log.organization_id || ''}","${log.user_email || ''}","${log.action}","${log.entity_type || ''}","${log.entity_id || ''}","${log.metadata?.description || ''}","${log.ip_address || ''}","${log.created_at}"\n`;
    });
  ...
  async getStatistics(organizationId?: string) {
    ...
    if (organizationId) {
      queryBuilder.where('audit_log.organization_id = :organizationId', { organizationId });
    }
