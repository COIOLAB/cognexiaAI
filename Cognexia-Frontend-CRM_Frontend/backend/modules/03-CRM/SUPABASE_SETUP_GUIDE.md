...
# Verify organization_id column exists in all tables
SELECT table_name 
...
-- Should only return rows where organization_id matches

-- Try to insert with wrong organization_id
INSERT INTO customers (id, organization_id, company_name, ...) 
...
USING (
  ...
  AND (storage.foldername(name))[1] = current_setting('app.current_organization_id', true)
);
...
CREATE INDEX CONCURRENTLY idx_customers_org_id ON customers(organization_id);
CREATE INDEX CONCURRENTLY idx_leads_org_id ON leads(organization_id);
CREATE INDEX CONCURRENTLY idx_opportunities_org_id ON opportunities(organization_id);
-- Repeat for all 32 tables...
...
EXPLAIN ANALYZE SELECT * FROM customers WHERE organization_id = 'xxx';
