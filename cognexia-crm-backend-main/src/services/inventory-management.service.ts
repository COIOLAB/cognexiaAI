...
export class InventoryManagementService {
  ...
  async getStockLevels(organizationId: string) {
    return this.stockRepo.find({ where: { organizationId: organizationId } });
  }

  async updateStockLevel(productId: string, data: any, organizationId: string) {
    const stock = await this.stockRepo.findOne({ where: { productId, organizationId: organizationId } });
    if (stock) {
      await this.stockRepo.update(stock.id, { ...data, lastUpdated: new Date() });
      return this.stockRepo.findOne({ where: { id: stock.id } });
    }
    const newStock = this.stockRepo.create({ ...data, productId, organizationId: organizationId, lastUpdated: new Date() });
    return this.stockRepo.save(newStock);
  }

  async getWarehouses(organizationId: string) {
    return this.warehouseRepo.find({ where: { organizationId: organizationId } });
  }

  async createWarehouse(data: any, organizationId: string) {
    try {
  ...
  async transferStock(data: any, organizationId: string) {
    const transfer = this.transferRepo.create({ ...data, organizationId: organizationId });
    return this.transferRepo.save(transfer);
  }

  async getReorderPoints(organizationId: string) {
    return this.reorderRepo.find({ where: { organizationId: organizationId } });
  }

  async setReorderPoint(data: any, organizationId: string) {
    const reorder = this.reorderRepo.create({ ...data, organizationId: organizationId });
    return this.reorderRepo.save(reorder);
  }

  async performAudit(warehouseId: string, data: any, organizationId: string) {
    const audit = this.auditRepo.create({ ...data, warehouseId, organizationId: organizationId, auditDate: new Date() });
  ...
  async getItem(id: string, organizationId: string) {
    return this.stockRepo.findOne({ where: { id, organizationId: organizationId } });
  }

  async createItem(data: any, organizationId: string) {
    try {
      const warehouseId =
        data.warehouseId || (await this.getOrCreateDefaultWarehouseId(organizationId));
  ...
  async updateItem(id: string, data: any, organizationId: string) {
    await this.stockRepo.update({ id, organizationId: organizationId }, { ...data, lastUpdated: new Date() });
    return this.stockRepo.findOne({ where: { id, organizationId: organizationId } });
  }

  async addItemLocation(itemId: string, data: any, organizationId: string) {
    const existing = await this.stockRepo.findOne({ where: { id: itemId, organizationId: organizationId } });
    if (!existing) {
      return null;
    }
    if (!data.warehouseId) {
  ...
  async recordMovement(itemId: string, data: any, organizationId: string) {
    const existing = await this.stockRepo.findOne({ where: { id: itemId, organizationId: organizationId } });
    if (!existing) {
      return null;
    }
  ...
  async createReplenishmentOrder(data: any, organizationId: string) {
    try {
      const defaultWarehouseId = await this.getOrCreateDefaultWarehouseId(organizationId);
      const transfer = this.transferRepo.create({
        organizationId: organizationId,
  ...
  private async getOrCreateDefaultWarehouseId(organizationId: string) {
