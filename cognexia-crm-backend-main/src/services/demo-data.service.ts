...
export class DemoDataService {
  ...
  private async clearStoryData(demoUserId: string) {
    ...
    await Promise.all([
      ...
      this.taskRepository.delete({ organizationId: DEMO_ORG_ID } as any),
      this.contactRepository.createQueryBuilder()
        .delete()
        .where('email LIKE :pattern', { pattern: '%@%-demo.com' })
        .execute(),
      this.activityRepository.delete({ organizationId: DEMO_ORG_ID } as any),
      this.noteRepository.delete({ organizationId: DEMO_ORG_ID } as any),
    ]);
  }
