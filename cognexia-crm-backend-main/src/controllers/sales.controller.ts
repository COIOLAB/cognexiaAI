...
export class SalesController {
  ...
  @ApiQuery({ name: 'status', required: false, enum: OpportunityStatus })
  ...
  async createOpportunity(@Body() createOpportunityDto: any, @Req() req: any) {
    try {
      const opportunity = await this.salesService.createOpportunity(
        createOpportunityDto,
        'system_user', // In real implementation, get from JWT token
        req?.user?.organizationId || createOpportunityDto.organizationId
      );
