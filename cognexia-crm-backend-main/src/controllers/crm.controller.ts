...
export class CRMController {
  ...
  async createCustomer(@Body() createCustomerDto: any, @Req() req: any) {
    try {
      const actor = req?.user?.id || req?.user?.userId || 'system_user';
      const organizationId = req?.user?.organizationId || createCustomerDto.organizationId || null;

      const payload = {
        ...createCustomerDto,
        organizationId,
        createdBy: createCustomerDto.createdBy || actor,
        updatedBy: createCustomerDto.updatedBy || actor,
      };

      const customer = await this.customerService.createCustomer(payload, actor, organizationId);
      return { success: true, data: customer, message: 'Customer created successfully' };
    } catch (error) {
      this.logger.error('Error creating customer:', error);
      if (error instanceof HttpException) throw error;
      throw new HttpException(error?.message || 'Failed to create customer', HttpStatus.BAD_REQUEST);
    }
  }
  ...
  @ApiOperation({ summary: 'Export customers', description: 'Export customers to CSV format based on provided filters' })
  ...
  async exportCustomers(
    ...
    try {
      const take = 10000; // max export limit
      ...
      const headers = ['ID', 'Name', 'Email', 'Customer Type', 'Status', 'Segment', 'Tier', 'Industry', 'Region', 'Revenue', 'Created At'];
      const csvRows = result.data.map((c: any) => [
        c.id,
        `"${(c.name || '').replace(/"/g, '""')}"`,
        `"${(c.email || '').replace(/"/g, '""')}"`,
        c.customerType || '',
  ...
  @Get('customers/stats')
  @ApiOperation({ summary: 'Get customer stats', description: 'Retrieve aggregated customer statistics' })
  ...
  async getCustomerStats() {
    try {
      ...
      return { success: true, data: stats, message: 'Customer stats retrieved successfully' };
    } catch (error) {
      this.logger.error('Error getting customer stats:', error);
      throw new HttpException('Failed to retrieve customer stats', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  @Get('customers/:id')
  @ApiOperation({ summary: 'Get customer by ID', description: 'Retrieve a specific customer' })
  ...
  @Roles('admin', 'manager', 'sales_manager', 'sales_rep', 'marketing')
  async updateCustomer(@Param('id') id: string, @Body() updateCustomerDto: any, @Req() req: any) {
    try {
      const actor = req?.user?.id || req?.user?.userId || 'system_user';
      const payload = {
        ...updateCustomerDto,
        organizationId: updateCustomerDto.organizationId || req?.user?.organizationId || null,
        updatedBy: actor,
      };
      const updated = await this.customerService.updateCustomer(id, payload, actor);
      return { success: true, data: updated, message: 'Customer updated successfully' };
    } catch (error) {
      this.logger.error(`Error updating customer ${id}:`, error);
      if (error instanceof HttpException) throw error;
      throw new HttpException(error?.message || 'Failed to update customer', HttpStatus.BAD_REQUEST);
    }
  }

  @Delete('customers/:id')
  @ApiOperation({ summary: 'Delete customer', description: 'Soft-delete (inactivate) a customer' })
  @ApiParam({ name: 'id', description: 'Customer UUID' })
  ...
  async deleteCustomer(@Param('id') id: string, @Req() req: any) {
    try {
      const actor = req?.user?.id || req?.user?.userId || 'system_user';
      const result = await this.customerService.deleteCustomer(id, actor);
      return { success: true, ...result };
    } catch (error) {
      this.logger.error(`Error deleting customer ${id}:`, error);
      if (error instanceof HttpException) throw error;
      throw new HttpException(error?.message || 'Failed to delete customer', HttpStatus.BAD_REQUEST);
    }
  }
  ...
  async getAllLeads(
    ...
    try {
      ...
      const result = await this.leadService.findAll({
        ...
        assignedTo,
        search,
        organizationId: req?.user?.organizationId || (req as any)?.organizationId,
      } as any);
  ...
  @Get('leads/export')
  @ApiOperation({ summary: 'Export leads', description: 'Export leads to CSV format based on provided filters' })
  ...
  async exportLeads(
    ...
    try {
      ...
      const headers = ['ID', 'Lead Code', 'Full Name', 'Email', 'Company', 'Phone', 'Status', 'Source', 'Score', 'Created At'];
      const csvRows = result.data.map((lead: any) => {
        ...
        return [
          ...
          `"${(lead.company || contact.company || '').replace(/"/g, '""')}"`,
  ...
  @Roles('admin', 'manager', 'sales_manager', 'sales_rep', 'marketing')
  async updateLead(@Param('id') id: string, @Body() updateLeadDto: any, @Req() req: any) {
    try {
      ...
      const payload = {
        ...
        organizationId: updateLeadDto.organizationId || req?.user?.organizationId || existing.organizationId,
      };
  ...
  @Roles('admin', 'manager', 'sales_manager', 'sales_rep', 'marketing')
  async createLead(@Body() createLeadDto: any, @Req() req: any) {
    try {
      ...
      const lead = await this.leadService.create({
        ...
        organizationId: createLeadDto.organizationId || req?.user?.organizationId,
      });
