...
- Each row has a `tenant_id` or `organizationId` column
...
```sql
-- CRITICAL: Add indexes on organizationId for ALL tables
...
// Server enriches JWT with organizationId
...
async createCustomer(organizationId: string, data: CreateCustomerDto) {
  return await this.customerRepository.save({
    ...data,
    organizationId: organizationId // Always inject tenant ID
  });
}
```
...
async checkQuota(organizationId: string, type: 'users' | 'storage') {
  ...
  if (type === 'users') {
    const userCount = await this.userRepository.count({ organizationId: organizationId });
...
UPDATE leads SET organizationId = 'default-org-id' WHERE organizationId IS NULL;
